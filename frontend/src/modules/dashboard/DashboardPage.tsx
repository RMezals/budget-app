    ...
    <div>
        ...
        <div className="col-sm-6 col-lg-3">
          <div className="card h-100 border-0 shadow-sm">
            <div className="card-body">
              ...
              <p className={`fs-4 fw-bold mb-0 ${netForMonth >= 0 ? 'text-success' : 'text-danger'}`}>
                {netForMonth >= 0 ? '+' : ''}{fmt(netForMonth)}
              </p>
              <p className="text-muted small mb-0">{fmt(summary.monthlyIncome)} in · {fmt(summary.monthlyExpenses)} out</p>
            </div>
          </div>
        </div>
      </div>
                ...
                {summary.budgetUsage.map(b => (
                  <div key={b.category} className="mb-3">
                    <div className="d-flex justify-content-between mb-1">
                      ...
                      <span className="small text-muted">{fmt(b.spent)} / {fmt(b.limit)}</span>
                    </div>
                ...
                {summary.activeGoals.map(g => (
                  <div key={g.goalId} className="mb-3">
                    <div className="d-flex justify-content-between mb-1">
                      ...
                      <span className="small text-muted">{fmt(g.currentAmount)} / {fmt(g.targetAmount)}</span>
                    </div>
                    <div className="progress" style={{ height: 8 }}>
                      <div
      ...
      <div className="card border-0 shadow-sm mt-4">
        <div className="card-body">
          ...
          <p className="text-muted small mb-3">Get personalised tips based on your current finances and goals.</p>
          ...
          {advisorError && (
            <div className="alert alert-danger mb-0">
              <p className="mb-0"><strong>Error:</strong> {advisorError}</p>
            </div>
          )}
          {advisor && (
            <div className="alert alert-light border mb-0">
              <p className="text-muted small mb-2">via {advisor.provider}</p>
              <p className="mb-0" style={{ whiteSpace: 'pre-wrap' }}>{advisor.tips}</p>
            </div>
          )}
        </div>
      </div>
